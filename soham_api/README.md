...
```bash
soham_api$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
soham_api$ sam local start-api
soham_api$ curl http://localhost:3000/
```
...
```bash
soham_api$ sam logs -n HelloWorldFunction --stack-name soham_api --tail
```
...
```bash
sam delete --stack-name soham_api
```
